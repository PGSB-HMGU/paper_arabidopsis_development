################################################
### Preprocessing - Developmental set flower ###
###############################################
...
# load MaxQuant proteinGroups.txt Developmental set flower
...
tmt_flower <- as.matrix(tmtprot[,grep('Reporter intensity [0-9]{1,2} DV_', colnames(tmtprot)),with=F])
cnames <- colnames(tmt_flower)
tmt_flower[tmt_flower==0] <- NA
tmt_flower_log2 <- log2(tmt_flower)
tmt_flower_log2 <- mediancenter(tmt_flower_log2)
tmtprot[,eval(cnames):=as.data.table(tmt_flower_log2)]
...
fwrite(tmtprot, paste0("/path/to/", "preproc_proteinGroups_DV_FL.txt"), sep = '\t', col.names = T, row.names = F)

# load MaxQuant phospho (STY)sites.txt Developmental set flower
...
tmtp_flower <- as.matrix(tmtphos[,grep('Reporter intensity [0-9]{1,2} P_', colnames(tmtphos)),with=F])
cnames <- colnames(tmtp_flower)
tmtp_flower[tmtp_flower==0] <- NA
tmtp_flower_log2 <- log2(tmtp_flower)
tmtphos[,eval(cnames):=as.data.table(tmtp_flower_log2)]

fwrite(tmtphos, paste0("/path/to/", "proproc_phosphoSTYsites_DV_FL.txt"),sep = '\t', col.names = T, row.names = F)
